...
public abstract class CmdAbstractListing extends FtpCmd {
    ...
    public String listDirectory(StringBuilder response, File dir) {
        ...
        Cat.d(TAG, "Listing directory: " + dir.toString());
        ...
        Cat.d(TAG, "Dir len " + entries.length);
        try {
            ...
            Cat.e(TAG, "Unable to sort the listing: " + e.getMessage());
    ...
    protected String sendListing(String listing) {
        if (sessionThread.openDataSocket()) {
            Cat.d(TAG, "LIST/NLST done making socket");
        } else {
        ...
        Cat.d(TAG, "Sent code 150, sending listing string now");
        if (!sessionThread.sendViaDataSocket(listing)) {
            Cat.d(TAG, "sendViaDataSocket failure");
            sessionThread.closeDataSocket();
            return "426 Data socket or network error\r\n";
        }
        sessionThread.closeDataSocket();
        Cat.d(TAG, "Listing sendViaDataSocket success");
